...
use auth::{
    ...
    server::server as auth_server,
    server_state::AuthServerState,
    AUTH_SESSION_COOKIE,
};
use http::{header::SET_COOKIE, HeaderMap, HeaderValue};
use runner::{self, RunnerConfig, RunnerConfigDeserialize, MAIN_SERVER_PORT};
...
fn session_id_cookie_value(headers: &HeaderMap<HeaderValue>) -> Option<Uuid> {
...
fn remove_session_id_from_cookie(upstream_request: &mut RequestHeader) {
...
pub mod tests {
    ...
    fn test_session_id_cookie_value() {
        ...
        let expected_session_id = Uuid::from_str("59daca20-29cd-489b-8a2a-b4f9da81607c").unwrap();
